...
public class AddStatistic extends CommandL {
    ...
    public AddStatistic(StatisticsRewards plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (statistic.getType().equals(Statistic.Type.ITEM) || statistic.getType().equals(Statistic.Type.BLOCK)) {
            ...
            player.setStatistic(statistic, material, player.getStatistic(statistic, material)+amount);
            sender.sendMessage(plugin.getLangManager().getLangValue(LangOption.STATISTIC_ADD, plugin.getPlayerUtil().getPlayer(sender)).replace("%player%", player.getName()).replace("%statistic%", StringUtils.capitalize(statistic.name().toLowerCase())).replace("%material/entity%", StringUtils.capitalize(material.name().toLowerCase())).replace("%amount%", String.valueOf(player.getStatistic(statistic, material))));
            ...
            player.setStatistic(statistic, entityType, player.getStatistic(statistic, entityType)+amount);
            ...
            player.setStatistic(statistic, player.getStatistic(statistic)+amount);
            sender.sendMessage(plugin.getLangManager().getLangValue(LangOption.STATISTIC_ADD_UNTYPED, plugin.getPlayerUtil().getPlayer(sender)).replace("%player%", player.getName()).replace("%statistic%", StringUtils.capitalize(statistic.name().toLowerCase())).replace("%amount%", String.valueOf(player.getStatistic(statistic))));
        }
    }
